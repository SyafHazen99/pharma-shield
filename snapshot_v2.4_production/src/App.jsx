...
function MainApp() {
  ...
  const handleRedistributeStock = (transferPayload) => {
    ...
    let targetDepoName = typeof transferPayload === 'object' ? transferPayload.targetDepo : 'Depo IGD 24 Jam';
        ...
        <main className="flex-1 p-4 lg:p-8 overflow-y-auto">
          ...
          <div className="mb-6 p-4 bg-white border-2 border-blue-500 shadow-md shadow-blue-500/10 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs font-sans">
            ...
            <span className="px-3 py-1.5 rounded-xl font-bold font-mono text-[11px] bg-amber-50 text-amber-900 border border-amber-300">
              {activeRole === 'WAREHOUSE' && '🔒 Akses RBAC Resmi: Stage 1 (Monitoring Stok) & Stage 5 (Goods Receipt OCR)'}
              {activeRole === 'PHARMACY_HEAD' && '🔒 Akses RBAC Resmi: Stage 2 (Purchase Request EOQ) & Stage 6 (Stock Update FEFO)'}
              {activeRole === 'PURCHASING' && '🔒 Akses RBAC Resmi: Stage 4 (Purchase Order & Vendor Price Benchmarking)'}
              {activeRole === 'DIRECTOR' && '🔑 Akses Executive Super Admin: Stage 0 (BI Dashboard), Stage 3 (Approval), & Stage 7 (3-Way Payment)'}
            </span>
          </div>
