...
export default function Sidebar({ activeStage, setActiveStage, activeRole, pendingCounts }) {
  const currentRoleConfig = ROLES[activeRole] || ROLES.DIRECTOR;

  // Under strict RBAC based on dr. Novia's pipeline, filter stages per role
  const visibleStages = ALL_STAGES.filter(stage => isStageAuthorized(activeRole, stage.id));
