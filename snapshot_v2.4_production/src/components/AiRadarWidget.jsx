...
export default function AiRadarWidget() {
  const [pulseScore, setPulseScore] = useState(99.4);
  const [packetsProcessed, setPacketsProcessed] = useState(14820);
  const [activeRadarAngle, setActiveRadarAngle] = useState(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setPulseScore(prev => +(99.2 + Math.random() * 0.7).toFixed(1));
      setPacketsProcessed(prev => prev + Math.floor(Math.random() * 3) + 1);
      setActiveRadarAngle(prev => (prev + 4) % 360);
    }, 1000);
    return () => clearInterval(interval);
  }, []);

  return (
    <div className="p-5 rounded-3xl border border-blue-200 bg-gradient-to-r from-blue-50/80 via-white to-slate-50 shadow-md shadow-blue-500/5 relative overflow-hidden space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 relative z-10">
        ...
        <div className="flex items-center gap-3">
          <div className="relative w-12 h-12 rounded-2xl bg-blue-600 border border-blue-400 flex items-center justify-center shrink-0 shadow-md shadow-blue-500/20 overflow-hidden">
            {/* Spinning Radar Line */}
            <div 
              className="absolute inset-0 bg-gradient-to-tr from-white/40 via-transparent to-transparent origin-center"
              style={{ transform: `rotate(${activeRadarAngle}deg)` }}
            ></div>
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono bg-emerald-100 text-emerald-800 border border-emerald-300 font-bold flex items-center gap-1">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 font-mono text-xs">
          ...
          <div className="p-2.5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-0.5">
            ...
            <strong className="text-blue-700 font-bold text-sm font-mono">{pulseScore}% Valid</strong>
          </div>
          ...
          <div className="p-2.5 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-0.5">
            ...
            <strong className="text-emerald-700 font-bold text-sm font-mono">{packetsProcessed.toLocaleString('id-ID')}</strong>
          </div>
