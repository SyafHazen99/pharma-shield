...
// Google Sheets Live Sync Configuration & Webhook Dispatcher
let googleSheetsWebhookUrl = process.env.GOOGLE_SHEETS_WEBHOOK_URL || '';

async function syncToGoogleSheets(itemPayload) {
  ...
  try {
    ...
    console.log(`[Google Sheets Sync] Successfully pushed '${itemPayload.name || itemPayload.drugName || 'Item'}' to Google Sheets Webhook.`);
  } catch (err) {
...
router.get('/info', (req, res) => {
  res.json({
    ...
    branding: BRANDING,
    timestamp: getWIBTimestamp(),
    googleSheetsConnected: Boolean(googleSheetsWebhookUrl)
  });
});
...
router.get('/google-sheets/config', (req, res) => {
  res.json({
    ...
    isConnected: Boolean(googleSheetsWebhookUrl)
  });
});
...
router.post('/google-sheets/config', (req, res) => {
  ...
  if (typeof webhookUrl === 'string') {
    ...
    addAuditLog('GOOGLE_SHEETS_WEBHOOK_UPDATED', `Google Sheets Webhook URL diset ke: ${googleSheetsWebhookUrl || 'Disconnected'}.`, 'INFO');
    res.json({ success: true, isConnected: Boolean(googleSheetsWebhookUrl), webhookUrl: googleSheetsWebhookUrl });
  } else {
    res.status(400).json({ error: "Invalid webhookUrl payload" });
  }
});
...
router.post('/google-sheets/sync', async (req, res) => {
  ...
  res.json({ success: true, timestamp: getWIBTimestamp() });
});
...
router.post('/medicines', async (req, res) => {
  ...
  if (!newMed || !newMed.name) {
    return res.status(400).json({ error: "Invalid medicine payload" });
  }

  const sanitizedMed = {
    id: sanitizeString(newMed.id) || `MED-2026-${Math.floor(1000 + Math.random() * 9000)}`,
    name: sanitizeString(newMed.name),
    dosage: sanitizeString(newMed.dosage) || 'Standard Spec',
    category: sanitizeString(newMed.category) || 'General',
    itemType: newMed.itemType === 'ALKES_BMHP' ? 'ALKES_BMHP' : 'MEDICINE',
    currentStock: Math.max(0, Number(newMed.currentStock) || 0),
    unit: sanitizeString(newMed.unit) || 'Pcs',
    minSafetyStock: Math.max(0, Number(newMed.minSafetyStock) || 50),
    reorderPoint: Math.max(0, Number(newMed.reorderPoint) || 75),
    eoq: Math.max(0, Number(newMed.eoq) || 150),
    unitPrice: Math.max(0, Number(newMed.unitPrice) || 0),
    burnRateDaily: Math.max(0.1, Number(newMed.burnRateDaily) || 5.0),
    abnormalUsageSpike: Boolean(newMed.abnormalUsageSpike),
    batchNo: sanitizeString(newMed.batchNo) || `BATCH-${Math.floor(100 + Math.random() * 900)}`,
    expiryDate: sanitizeString(newMed.expiryDate) || '2027-12-31',
    location: sanitizeString(newMed.location) || 'Gudang Utama',
    status: 'Normal',
    fefoRank: Number(newMed.fefoRank) || 2,
    supplierId: sanitizeString(newMed.supplierId) || 'SUP-001'
  };

  db.medicines.unshift(sanitizedMed);
  addAuditLog('MANUAL_MEDICINE_ENTRY', `Obat/Alkes baru ${sanitizedMed.name} (${sanitizedMed.dosage}) didaftarkan ke sistem.`, 'INFO');
  saveDB();

  // Async push to Google Sheets Webhook
  syncToGoogleSheets({
    ...sanitizedMed,
    timestamp: getWIBTimestamp(),
    actor: 'PharmaShield App'
  });

  res.status(201).json(sanitizedMed);
});
...
router.post('/purchase-requests', async (req, res) => {
  ...
  syncToGoogleSheets({
    ...
    id: newPR.id,
    name: newPR.drugName,
    requestedQty: newPR.requestedQty,
    unit: newPR.unit,
    estimatedTotal: newPR.estimatedTotal,
    requestedBy: newPR.requestedBy,
    riskScore: newPR.riskScore,
    timestamp: newPR.requestDate
  });
